 ...
 */
async function fetchClients(page = 1, limit = 50, search = '', searchField = 'all', searchType = 'contains', sort = 'last_name_asc') {
    try {
        ...
        const response = await fetch(`../../../src/Api/api.php?endpoint=clients&${params.toString()}`);
...
async function deleteClient(clientId) {
    try {
        const response = await fetch(`../../../src/Api/api.php?endpoint=clients&id=${clientId}`, {
            method: 'DELETE'
        });
        const data = await response.json();
        return data;
    ...
    if (popupWindow) {
        ...
        const checkClosed = setInterval(() => {
            if (popupWindow.closed) {
                ...
                console.log('New client popup closed, reloading list');
                loadClients();
            }
        }, 500);
    } else {
        console.error('Failed to open new client popup window');
        alert('Impossibile aprire la finestra popup. Controlla le impostazioni del browser.');
    }
}
